...
function Row({ icon, label, value, onClick, soon }: { icon: React.ReactNode; label: string; value?: string; onClick?: () => void; soon?: boolean }) {
  return (
    <button onClick={onClick} disabled={!onClick} className="flex w-full items-center gap-3 px-4 py-3.5 text-left">
      ...
      {soon ? (
        <span className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-mute">Coming soon</span>
      ) : (
...
export function ProfileScreen() {
  ...
  const service = (ok: boolean | undefined, name: string) => (
    <div className="flex items-center justify-between px-4 py-3 text-[13.5px]">
      ...
      {status === null ? (
        <span className="text-mute">Checking…</span>
      ) : ok ? (
        <span className="flex items-center gap-1 font-semibold text-[#0d8a6a]">
          <CircleCheck size={15} /> Connected
        </span>
      ) : (
      ...
      <GlassPanel className="mt-2 divide-y divide-white/70">
        <button
          onClick={() => {
            ...
            toast("Demo week loaded. Your visits were cleared.");
          }}
          className="flex w-full items-center gap-3 px-4 py-3.5 text-left text-[14.5px] font-medium text-ink"
        >
          <RotateCcw size={18} className="text-indigo" /> Try Demo — reset to sample week
        </button>
        <button
          onClick={() => {
            if (window.confirm("Clear all health notes and visits from this device?")) {
