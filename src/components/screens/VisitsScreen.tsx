    ...
    <ScreenScroll>
      <header className="flex items-start justify-between">
        <div>
          ...
          <h1 className="mt-1 text-[30px] font-bold leading-tight tracking-[-0.025em] text-ink">Your consultations</h1>
        </div>
      ...
      <PrismCard className="mt-5 p-5">
        <div className="flex items-center gap-3">
          <Ada size={78} />
          <p className="text-[17px] font-semibold leading-snug tracking-tight text-ink">CareEcho can record and remember your consultation.</p>
        </div>
        <ul className="mt-4 space-y-2.5 text-[13px] text-ink-soft">
          <li className="flex gap-2.5">
            <Mic size={16} className="mt-0.5 shrink-0 text-indigo" /> Transcribed by AssemblyAI, with speaker labels.
          </li>
          <li className="flex gap-2.5">
            <FileSearch size={16} className="mt-0.5 shrink-0 text-indigo" /> Ada saves medications, follow-ups and instructions — each linked to the exact words.
          </li>
          <li className="flex gap-2.5">
            <Lock size={16} className="mt-0.5 shrink-0 text-indigo" /> Stored only on this device.
          </li>
        </ul>
        ...
        >
          ...
          <span className="text-[13.5px] font-medium leading-snug text-ink">Make sure everyone being recorded has agreed.</span>
        </button>
        ...
        >
          <AudioLines size={19} /> Start visit
        </motion.button>
        <SampleButton />
      </PrismCard>
      ...
      {visits.length > 0 && (
        <section className="mt-6">
          <p className="eyebrow">Saved visits</p>
                ...
                <button onClick={() => go({ name: "visit", id: v.id })} className="glass flex w-full items-center gap-3 rounded-[22px] p-3.5 text-left">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11.5px] text-ink-soft">
                      {v.facts.medications.length + v.facts.instructions.length + (v.facts.followUp ? 1 : 0)} saved items · {v.qa.length} questions
                    </p>
                  </div>
      ...
      <GlassPanel className="mt-6 p-4">
        <p className="text-[13px] font-semibold text-ink">No source → no claim</p>
        <p className="mt-1 text-[12px] leading-relaxed text-mute">
          Ada only keeps what your clinician actually said. Things you said stay marked as yours — they never become instructions.
        </p>
      </GlassPanel>
...
function SampleButton() {
  ...
  return (
    ...
    >
      <FlaskConical size={14} /> Load demo recording (sample consultation)
    </button>
  );
}
